<?php
//blog
function pr_theme_blog_1_column($atts, $content="", $component="blog_1_columns")
{
	global $themename;
	if(isset($_GET["atts"]))
	...
	$args = array( 
		'post__in' => $ids,
		'post__not_in' => ($featured_post!="-" ? array((int)$featured_post, get_the_ID()) : array(get_the_ID())),
		'post_type' => 'post',
		'post_status' => 'publish',
		'paged' => $paged,
	...
	if(have_posts())
	{
		...
		while (have_posts()) : the_post();
			$post = new Pr_Post("1_column", $featured_post, get_post_meta(get_the_ID(), $themename. "_is_review", true), get_post_format(get_the_ID()), $featured_image_size, (int)$show_post_icon, (int)$show_post_date, (int)$show_post_categories, (int)$show_post_excerpt, (int)$show_post_author, $post_details_layout, "", $category, $i, $themename);
			if($i==0 || ($featured_post!="-" && $i==1))
			{
				if($featured_post!="-" && $i==1)
					$output .= '</ul>';
				$output .= '<ul class="blog clearfix' . ($featured_post!="-" && $i==1 ? " small" : " big") . ($top_margin!="none" ? ' ' . $top_margin : '') . ($el_class!="" ? ' ' . $el_class : '') . '">';
			}
...
//ajax pagination
add_action("wp_ajax_theme_blog_1_column_pagination", "pr_theme_blog_1_column");
add_action("wp_ajax_nopriv_theme_blog_1_column_pagination", "pr_theme_blog_1_column");

//visual composer
function pr_theme_blog_1_column_vc_init()
{
	//get posts list
	...
	vc_map( array(
		"name" => __("Blog 1 Column", 'pressroom'),
		"base" => "blog_1_column",
		"class" => "",
		"controls" => "full",
...
add_action("init", "pr_theme_blog_1_column_vc_init");
?>
